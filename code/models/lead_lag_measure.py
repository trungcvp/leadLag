...
# Thay đổi cách tính lead_lag - có bổ sung thêm tích hợp trọng số VOLUMNE

def compute_lead_lag(data_subset, method, volume_df = None, **kwargs):
    '''
    ...
    if method == 'ccf_at_lag':
        ...
        if volume_df is not None:
            col_x = data_subset.columns[0]
            col_y = data_subset.columns[1]
            vol_x = volume_df[col_x].mean()
            vol_y = volume_df[col_y].mean()
        else:
            vol_x = vol_y = 1  # mặc định không trọng số

        
        A = np.abs(cross_correlation_measure[cross_correlation_measure.index > 0]).sum() * vol_x
        B = np.abs(cross_correlation_measure[cross_correlation_measure.index < 0]).sum() * vol_y
...
def lead_lag_given_pair(args):
    pair, data, method, volume_df,kwargs_compute_lead_lag = args
    
    data_subset = data.loc[:, pair]
    
    return compute_lead_lag(data_subset, method=method,volume_df= volume_df, **kwargs_compute_lead_lag)


def get_lead_lag_matrix(data, method,volume_df, **kwargs_compute_lead_lag):
    pair_list = list(itertools.combinations(data.columns, 2))
    args = list(itertools.product(pair_list, [data], [method], [volume_df], [kwargs_compute_lead_lag]))
